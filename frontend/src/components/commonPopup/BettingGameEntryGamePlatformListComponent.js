    ...
    return (
        <IonModal
            ...
            {(userInfo?.role !== 1) ?
                        ...
                        <div className={"infiniteScroll"}>
                            <div className={"infiniteScroll__loading"}>
                                {loading ?
                                    ...
                                    : (gameSessionData?.id) ?
                                        <div className={"sysMessage__container for_betting_session_list"}>
                                            <div className="sysMessage__container-msgWrapper__item for_session">
                                                <div className="sysMessage__container-msgWrapper__item-title">
                                                    <div>
                                                        <span
                                                            className={"title"}><strong>Platform: {gameSessionData?.betting_platform_name}</strong></span>
                                                    </div>
                                                    {(gameSessionData?.is_active) ?
                                                      <span onClick={() => callFunctionToEnterBet(gameSessionData, gameSessionData?.betting_platform_id)} className={`action_button update`}>ENTER</span> :''
                                                    }
                                                </div>
                                                <div className="sysMessage__container-msgWrapper__item-time">
                                                {(gameSessionData?.is_active) ?
                                                        <div className={"active_session_message"}>
                                                            Session is active click on enter button
                                                        </div>
                                                         :
                                                    <div className={"active_session_message"}>
                                                        Session is not started yet by the admin please wait while admin
                                                        start this session
                                                    </div>
                                                }
                                                </div>
                                        ...
                                        <div className={"empty__container empty"}>
                                            <svg className={"svg-icon icon-empty"}
                ...
                :
                <GetBetGameSessionListPage handleCloseModal={handleCloseModal} callFunctionToActiveSectionAndStartGame={callFunctionToActiveSectionAndStartGame}/>
            }
        </IonModal>
    );
};
