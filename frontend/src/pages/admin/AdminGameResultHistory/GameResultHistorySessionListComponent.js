...
import {IonCol, IonContent, IonDatetime, IonHeader, IonIcon,IonPage, IonRow} from "@ionic/react";
...
export default function GameResultHistorySessionListComponent() {
    const dispatch = useDispatch();
    const history = useHistory();
    const {sessionData} = useSelector((state) => state.locallyStorePredictionHistoryData);
    ...
    const callFunctionToOpenSessionDetailPage = (data) => {
        data.session_date = sessionData?.session_date
        dispatch(actionToGetPredictionHistoryData(data, "prediction_data"));
        history.push("/game-result-period-list");
    }


    useEffect(() => {
        if (sessionData?.sessions?.length > 0) {
            // ✅ create updated sessions with sessionStatus
            const updatedSessions = sessionData.sessions.map(session => {
                let sessionStatus = "Verified";

                if (!session.game_results) {
                    sessionStatus = "Pending";
                } else if (session.game_results.some(result => result.result === null)) {
                    sessionStatus = "Pending";
                }
