...
export default function GameUsersOrderStatusDetailComponent() {
    const {usersOrderStatusDetail} = useSelector((state) => state.locallyStorePredictionHistoryData)
                ...
                <div className={"van-sticky"}>
                    <div className="foot_insub">
                        <div className="progress">
                            <div className="step">Date: {moment(usersOrderStatusDetail?.created_at).format('YYYY-MM-DD')}</div>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className={"bet-content__box"}>
                    <div className={"infiniteScroll"}>
                        <div className={"infiniteScroll__loading"}>
                            { (usersOrderStatusDetail) ?
                                    <div className={"sysMessage__container"}>
                                        <div className={"bet-container-sticky"}>
                                            <div className={"van-sticky"}>
                                                <div className="foot_insub">
                                                    <div className="progress">
                                                        <div className="step">User id: {usersOrderStatusDetail.uid} </div>
                                                        <div className="step">Order: {usersOrderStatusDetail.betting_balance} </div>
                                                        <div className="step">Executed: {usersOrderStatusDetail.amount ?? 0} </div>
                                                        <div className="step">Pending: {Number(usersOrderStatusDetail.betting_balance ?? 0) - Number(usersOrderStatusDetail.amount ?? 0)} </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
