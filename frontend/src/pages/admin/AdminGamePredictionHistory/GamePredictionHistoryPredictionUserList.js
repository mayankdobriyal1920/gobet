...
export default function GamePredictionHistoryPredictionUserList() {
    const allUserListObject = useSelector(state => state.allUserListObject)
    const history = useHistory();
    const {predictionHistoryUserList} = useSelector((state) => state.locallyStorePredictionHistoryData);
    const goBack = ()=>{
        history.goBack();
        window.history.back();
    }
